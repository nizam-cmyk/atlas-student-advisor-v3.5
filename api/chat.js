...
function getUnknownUploadResponse(fileMeta, extracted, classification, handbookContext) {
  ...
  const extractionNote = extracted?.success
    ? 'Some document content was extracted, but it was not strong enough for reliable form identification.'
    : (extracted?.reason || 'No readable document text could be extracted.');
...
Issue Summary:
I received your uploaded file${fileMeta?.filename ? ` (${fileMeta.filename})` : ''}, but I could not identify the document type automatically.

Possible Reason:
The uploaded PDF may be image-based or scanned, so readable text could not be extracted clearly enough.

Recommended Action:
1. Upload a clearer text-based PDF if available.
2. Or type the exact title shown at the top of the form.
3. Example:
   - "This is a Fee Review Request Form."
   - "This is an Academic Dismissal Appeal Form."
   - "This is a Course Withdrawal Form."
...
Important Note:
ATLAS can identify forms more reliably when the uploaded PDF contains selectable text, not only scanned images.
